...
void CBattle::Init() 
{
	...
	if (strMapFileName == "Village.xml" || strMapFileName == "Ling.xml") {
		...
		CSoundManager::getInstance()->StartBGM(BATTLE_MAP1_BGM);
	}
	else {
		fSpeed = 4;

		CSoundManager::getInstance()->StopBGM();
		CSoundManager::getInstance()->StartBGM(BATTLE_MAP2_BGM);
	}
	...
	CSoundManager::getInstance()->StartEffect(SOUND_READY);
}
	...
	if (bomber[0].player == PLAYER_P1) {
		if (GetAsyncKeyState(KEY_K) < 0 && dwCurTime - bomber[0].dwButtonTime > 190) { // 폭탄 설치
			...
			if (bCheck && bomber[0].dwMaxBombCount > nBombCount) {
				...
				CSoundManager::getInstance()->StartEffect(SOUND_SET_BOMB);
			}
		}
